...
import { isPCOrWeChat } from '@/utils/env'
...
async function init() {
  ...
  const code = getCode()
  const url = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${appid}&redirect_uri=${redirectUri.value}&response_type=code&scope=snsapi_base&state=12333#wechat_redirect`
  if (code) {
    // 有code，说明是从微信跳转过来的
    // 自动登录
...
onMounted(() => {
  ...
  if (typeof window !== 'undefined' && isPCOrWeChat()) {
    redirectUri.value = encodeURIComponent(window.location.href)
    init()
  }
})
</script>
